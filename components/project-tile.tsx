...
const ProjectTile = ({ project }: ProjectTileProps) => {
  const completeStatus = project.completed ? 
    <Button className='rounded-full w-fit cursor-default bg-green-500/60 hover:bg-green-500/60'>Complete</Button> 
      : 
    <Button className='rounded-full w-fit cursor-default bg-red-600/60 hover:bg-red-600/60'>Incomplete</Button>

  const dateIcons = project.finishDate ?
    <section className='flex gap-2'>
      <div className='flex gap-1 items-center bg-[#284d85] w-fit p-1 px-2 rounded-[8px] text-gray-100 font-light'>
  ...
  return (
    <div className='rounded-[12px] h-full min-h-[260px]'>
