...
const ProjectTile = ({ project }: ProjectTileProps) => {
  ...
  const projectImage = project.images?.[0] || '/fallback.jpg'

  return (
    <Link href={'/projects/' + project.id}>
      <div className='rounded-[12px] h-full min-h-[320px] overflow-hidden'>
        <div className='relative rounded-[12px] h-full flex flex-col justify-between p-4'>
          ...
          <div className='relative z-10 flex flex-col justify-between h-full'>
            <div className='flex flex-col gap-2'>
              { completeStatus }
              <h3 className='text-[#f0f1f1] drop-shadow-[0_1.2px_1.2px_rgba(128,128,128,0.6)] text-xl font-semibold'>
                { project.name }
              </h3>
            </div>
            ...
            <div className='flex justify-between items-center'>
              ...
              <Button className='bg-[#284d85] hover:bg-[#3D70BC] ease-in-out duration-200 p-4 hover:shadow-xl font-normal rounded-[8px] text-sm'>
                <p>See more</p>
                <Image alt='See more' src='/see_more.svg' height={20} width={20}/>
              </Button>
            </div>
          </div>

          <div className="absolute inset-0 bg-black/30 rounded-[12px] z-0" />
        </div>
      </div>
    </Link>
  )
