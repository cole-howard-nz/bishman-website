...
const ProjectTile = ({ project }: ProjectTileProps) => {
  const completeStatus = project.isComplete ? 
    <Button className='rounded-full w-fit cursor-default bg-green-500/60 hover:bg-green-500/60 p-2 h-6'>Complete</Button> 
      : 
    <Button className='rounded-full w-fit cursor-default bg-orange-600/60 hover:bg-orange-600/60 p-2 h-6'>Ongoing</Button>
    ...
    <div className='rounded-[12px] h-full min-h-[320px]'>
      <div className='rounded-[12px] bg-[url(/skytower.png)] bg-center bg-cover h-full flex flex-col justify-between p-4'>
        ...
        <div className='flex justify-between items-center'>
          ...
          <Button className='bg-[#284d85] hover:bg-[#3D70BC] ease-in-out duration-200 p-4 hover:shadow-xl font-normal rounded-[12px] text-sm'>
